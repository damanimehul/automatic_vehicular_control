...
with open(path+"/job.slurm","w") as f :
   ...
   # f.write('#SBATCH --output=$HOME/automatic_vehicular_control/slurm_jobs/test-1/%a_%x_%j.out\n'.format(experiment_name))
   # f.write('#SBATCH --error=$HOME/automatic_vehicular_control/slurm_jobs/test-1/%a_%x_%j.err\n'.format(experiment_name)) 
    f.write('#SBATCH --export=ALL\n') 
    f.write('#SBATCH --time=24:00:00\n')
    f.write('#SBATCH --mem=8G\n')
    f.write('#SBATCH --mail-type=END\n')
    f.write('#SBATCH --mail-user=mehul42@mit.edu\n')
    f.write('#SBATCH -c 1\n') 
    f.write('#SBATCH -N 1\n') 
